...
GlobalCtrl.$inject = ["$scope", "BadgeAlerts", "localStorageService", "$q", "$templateCache", "smoothScroll", "$interval", "notifications", "$http", "$anchorScroll", "Auth", "User", "$location", "$rootScope"];;
...
'use strict';
angular.module('snaptasqApp').controller('SettingsCtrl', ["$scope", "Task", "$window", "$location", "$timeout", "Modal", "User", "Auth", "notifications", "Notification", function($scope, Task, $window, $location, $timeout, Modal, User, Auth, notifications, Notification) {
    $scope.errors = {};
...
angular.module('snaptasqApp')
    .controller('AdminUsersCtrl', ["$scope", "$http", "Auth", "User", function($scope, $http, Auth, User) {
...
angular.module('snaptasqApp').controller('CommunitiesCtrl', ["$scope", "Community", "$http", "$window", function($scope, Community, $http, $window) {
    ...
    $scope.listCommunities();
}]).controller('CommunityCtrl', ["$scope", "Community", "Auth", "$routeParams", "Notification", "notifications", function($scope, Community, Auth, $routeParams, Notification, notifications) {
...
'use strict';
angular.module('snaptasqApp')
    .controller('TasksCtrl', ["$scope", "_me", "Page", "notifications", "Notification", "$location", "$window", "$routeParams", "Auth", "Task", "$timeout", "$interval", "User", "TaskMock", "KeyEventService", "TaskMarshaler", "Modal", "$rootScope", function($scope, _me, Page, notifications, Notification, $location, $window, $routeParams, Auth, Task, $timeout, $interval, User, TaskMock, KeyEventService, TaskMarshaler, Modal, $rootScope) {
    ...
    .controller('TaskEditCtrl', ["$scope", "$window", "Modal", "notifications", "$routeParams", "Task", "Notification", "$rootScope", "TaskMarshaler", "Auth", "$location", function($scope, $window, Modal, notifications, $routeParams, Task, Notification, $rootScope, TaskMarshaler, Auth, $location) {
        $scope._bgcolorGrey();
...
angular.module('snaptasqApp')
    .controller('NavbarCtrl', ["$scope", "$location", "Auth", "Notify", "TaskMarshaler", "$interval", "$timeout", function($scope, $location, Auth, Notify, TaskMarshaler, $interval, $timeout) {
...
angular.module('snaptasqApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/account/forgot/forgot.html',
    "<div class=container id=loginsAndSignup><section class=\"col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2\" style=margin-top:50px><div id=forgotbox class=mainbox><div class=\"panel panel-info\"><!-- <div class=\"panel-heading\">\n" +
    "                <div class=\"panel-title\">Sign In</div>\n" +
    "            </div>      --><div class=panel-body><!-- <div style=\"\" id=\"login-alert\" class=\"alert alert-danger col-sm-12\"></div> --><form id=forgotform class=form-horizontal name=forgotForm role=form ng-submit=sendForgotPassword(forgotForm) novalidate><div class=form-group><label for=email class=\"col-md-3 control-label\">Email</label><div class=col-md-9><input type=email class=form-control name=email ng-model=signupform.email placeholder=\"Email Address\"></div></div><div class=form-group><input type=hidden ng-model=captchaResponse name=captchaResponse><label for=captcha class=\"col-md-3 control-label\">Captcha</label><div class=col-md-9><div vc-recaptcha key=\"'6LfjmgcTAAAAAFerg8_alh6LFcb4QQvrF8Gg4qz1'\" on-success=\"captchaResponse=response\"></div></div></div><div class=\"col-md-12 form-group has-error\"><p class=help-block ng-show=\"form.email.$error.required && form.password.$error.required && submitted\">Please enter your email and password.</p><p class=help-block ng-show=\"form.email.$error.email && submitted\">Please enter a valid email.</p><p class=help-block>{{ errors.other }}</p></div><div class=form-group><!-- Button --><div class=\"col-sm-12 col-md-12 controls\"><button class=\"btn btn-success\" style=width:100% type=submit>Submit</button></div></div><div class=form-group><div class=\"col-md-12 control\"><div style=\"border-top: 1px solid#888; padding-top:15px; font-size:85%\">Don't have an account! <a href=\"/signin?action=register\" ng-click=\"tabSignup=0;tabCreateAccount=1\">Sign Up Here</a></div></div></div></form></div></div></div></section></div>"
  );
  ...
  $templateCache.put('app/admin/beta/admin.beta.html',
  ...
  $templateCache.put('app/admin/community/admin.community.html',
  ...
  $templateCache.put('app/admin/users/admin.users.html',
    "<div class=\"container top-buffer\"><h1>View Users</h1><ul class=list-group><li class=list-group-item ng-repeat=\"user in users\"><strong>{{user.name}}</strong><br><span class=text-muted>{{user.email}}</span><!-- <a ng-click=\"delete(user)\" class=\"trash\"><span class=\"glyphicon glyphicon-trash pull-right\"></span></a> --></li></ul></div>"
  );


  $templateCache.put('app/communities/communities.html',
    "<div class=container><section class=col-md-12 style=margin-top:50px><div class=\"panel panel-info\"><div class=panel-body><h1 style=display:inline class=text-center>Public Community Tasqs</h1><!-- <button class=\"btn btn-info pull-right\" ng-click=\"showSuggestCommunityModal()\"><i class=\"fa fa-plus\"></i>&nbsp;Suggest Community</button> --><ul><li ng-repeat=\"item in communities | filter:{status:'public'}\"><a href=/community/{{item._id}} class=lead>{{item.name}}</a></li></ul></div></div></section><section class=col-md-12 style=margin-top:50px><div class=\"panel panel-info\"><div class=panel-body><h1 style=display:inline class=text-center>Private Community Tasqs</h1><!-- <button class=\"btn btn-info pull-right\" ng-click=\"showSuggestCommunityModal()\"><i class=\"fa fa-plus\"></i>&nbsp;Suggest Community</button> --><ul><li ng-repeat=\"item in communities | filter:{status:'private'}\"><a href=/community/{{item._id}} class=lead>{{item.name}} Tasqs</a></li></ul></div></div></section></div>"
  );


  $templateCache.put('app/communities/community.html',
    "<div class=container><!-- group {{group}} --><div ng-if=\"allowed===false\" ng-include src=\"'app/communities/partials/join.community.html'\"></div><div ng-if=allowed ng-include src=\"'app/communities/partials/main.community.html'\"></div></div>"
  );


  $templateCache.put('app/communities/partials/join.community.html',
    "<div class=row ng-repeat=\"item in group.challenges\"><div class=\"col-md-12 text-center\"><p>{{item.prompt}}</p><form><input ng-model=\"creds\"/> <button class=btn ng-click=requestJoin(item,creds)>Join</button></form></div></div>"
  );


  $templateCache.put('app/communities/partials/main.community.html',
    "<div class=row><div class=col-md-12>{{group}}<div class=top-buffer><div ng-if=\"tasks.length==0\" class=\"well top-buffer\"><div class=task-view-compact-list-empty></div></div><div class=\"well top-buffer\" ng-repeat=\"task in tasks | searchTaskFilter:filter\"><div class=task-view-compact-list task=task></div></div></div></div></div>"
  );
  ...
  $templateCache.put('app/task/task.edit.html',
    "<div class=\"container top-buffer\" ng-controller=TaskEditCtrl><div class=row><div class=col-sm-12><p class=lead><strong style=padding-right:15px>tell us what tasq you need done</strong> <button ng-click=cancelEditingTask(task); type=button class=\"btn btn-warning\" style=height:46px>cancel</button></p></div></div><div style=background-color:white;padding-top:25px;padding-left:15px;padding-bottom:30px;padding-right:15px class=col-sm-12><form class=form name=form ng-submit=createTask(form) novalidate><div class=row><div class=\"col-md-6 col-sm-6 col-xs-12\"><div class=form-group ng-class=\"{ 'has-success': form.name.$valid && submitted,\n" +
    "                    'has-error': form.name.$invalid && submitted }\"><label>title</label><input scroll-on-click-mobile=-15px placeholder=\"give your tasq a descriptive title\" maxlength=128 name=name class=form-control ng-model=task.name required/><p class=help-block ng-show=\"form.name.$error.required && submitted\">a title is required</p></div><div class=form-group ng-class=\"{ 'has-success': form.name.$valid && submitted,\n" +
    "                'has-error': form.name.$invalid && submitted }\"><label popover-placement=right popover=\"It is reccomended you learn how to set price points accurately, clicking the link won't clear your task information\" popover-trigger=mouseenter>reward price</label><input scroll-on-click-mobile=-15px class=form-control format-as-currency=\"\" style=display:inline ng-model=task.payout popover-placement=right popover=\"By offering as little as 5$ it greatly increases the chances of success\" required/><div class=price-bar-container style=margin-top:5px><div class=price-bar-item ng-class=\"{'price-bar-filled':task.payout>0}\"></div><div class=price-bar-item ng-class=\"{'price-bar-filled':task.payout>10}\"></div><div class=price-bar-item ng-class=\"{'price-bar-filled':task.payout>20}\"></div></div><div class=pull-right><strong>price category:&nbsp;<a href=/pricePoints target=_tab>{{task.payout | pricePointCategory}}&nbsp;<i class=\"fa fa-question-circle\"></i></a></strong></div></div></div><!-- ./col-sm-6 --><div class=\"col-md-6 col-sm-6 col-xs-12\"><div class=form-group ng-class=\"{ 'has-success': form.description.$valid && submitted,\n" +
    "            'has-error': (form.description.$invalid || errors.description) && submitted }\"><label>additional details (optional)</label><textarea scroll-on-click-mobile=-15px placeholder=\"Ex: Couch is small enough to fit in an SUV.\" name=description cols=40 maxlength=1000 rows=5 ng-model=task.description class=form-control></textarea><p class=help-block ng-show=\"errors.description && submitted\">provide any extra details or requirements</p></div><div class=form-group ng-class=\"{ 'has-success': form.location.$valid && !errors.location && submitted,\n" +
    "        'has-error': (form.location.$invalid || errors.location) && submitted }\"><label>location (optional)</label><input scroll-on-click-mobile=-15px class=form-control name=location placeholder=\"enter a location\" ng-autocomplete ng-model=task.location.name options=options details=\"task.location.details\"/><p class=help-block ng-show=\"errors.location && submitted\">please re-enter the location</p></div><div class=\"row bot-buffer\"><div class=col-md-12><!-- Split button --><div class=\"btn-group pull-right\" dropdown ng-if=\"action=='create'\"><button id=split-button type=submit class=\"btn btn-primary btn-lg\"><span ng-if=\"!postTo || postTo=='private'\">POST PRIVATELY</span> <span ng-if=\"postTo=='community'\">POST TO COMMUNITY ({{countEnabledCommunities(communitiesList)}})</span></button> <button type=button class=\"btn btn-primary btn-lg\" dropdown-toggle><span class=caret></span></button><ul class=dropdown-menu ng-click=$event.stopPropagation() role=menu aria-labelledby=split-button><li role=menuitem ng-click=\"postTo='private';uncheckAllCommunities(communitiesList);\"><a href=\"\">POST PRIVATELY</a></li><li role=menuitem ng-click=\"postTo='community'\"><a href=\"\">POST TO COMMUNITY ({{countEnabledCommunities(communitiesList)}})</a></li><li class=divider></li><ul style=max-height:100px;overflow-y:auto;list-style:none;margin:0px;padding:0px;padding-left:5px;padding-right:5px><li role=menuitem ng-repeat=\"item in communitiesList\" ng-click=\"postTo='community'\"><a href=\"\" style=display:inline ng-click=\"item.active=!item.active\"><input type=checkbox ng-model=item.active style=\"display:inline;padding-left:10px\"/>{{item.name}}</a></li></ul></ul></div><button ng-if=\"action=='update'\" class=\"btn btn-success btn-lg pull-right\" type=submit>UPDATE TASQ!</button></div></div></div></div></form></div></div>"
  );


  $templateCache.put('app/task/task.view.html',
    "<div class=\"container top-buffer\"><div class=row><div class=\"col-md-7 col-sm-12\"><div class=\"row ng-cloak\" ng-if=\"_me._id && task.tasker.id==_me._id\"><div class=col-md-12><p ng-if=!task.tasker.confirmed class=\"lead text-info\">{{task.ownerName}} has chosen you to help. Do you accept? <button ng-if=!task.tasker.confirmed class=\"btn btn-success\" ng-click=taskerConfirmTask(task,true);>Accept</button> <button ng-if=!task.tasker.confirmed class=\"btn btn-error\" ng-click=taskerConfirmTask(task,false);>Reject</button></p></div></div><div class=row id=currentTasker ng-if=task.tasker.id><div class=col-md-12><div><img ng-if=task.tasker.pic src={{task.tasker.pic}} style=\"height:50px;width:50px;margin-right:5px\"/><a ng-if=task.tasker.fbId href=http://www.facebook.com/{{task.tasker.fbId}}>{{task.tasker.name}}</a><span ng-if=!task.tasker.fbId>{{task.tasker.name}} (<strong>Current <span ng-if=task.tasker.confirmed>active</span> Tasker</strong>)</span></div></div></div><div class=row id=buttonControls><div class=col-md-12><button ng-click=showApplicants(task) class=\"btn btn-info\" style=height:46px>View <span ng-if=\"task.applicants.length>0\">{{task.applicants.length}}</span> Applicants</button> <span ng-if=\"task.ownerId!=_me._id\"><button ng-if=canApplyToTask(task,_me); ng-click=applyToTask(task) class=\"btn btn-success\" style=height:46px>Apply to Task</button> <button ng-if=canUnapplyToTask(task,_me) ng-click=unapplyToTask(task) class=\"btn btn-warning\" style=height:46px>Unapply to Task</button></span><!-- task owner buttons --> <span ng-if=\"task.ownerId==_me._id\"><a href=/task/update/{{task._id}}><button class=\"btn btn-warning\" style=height:46px>Edit Task</button></a> <button ng-click=deleteTask(task) class=\"btn btn-danger\" style=height:46px>Delete Task</button></span></div></div><div class=\"row top-buffer\"><div class=col-md-12><div><div style=float:left;margin-right:10px><img src={{task.ownerPic}} style=\"height:50px;width:50px\"/></div><div><strong>{{task.ownerName}}</strong>&nbsp;<span>{{task.created | date:'MMM d, y h:mm a'}}</span>&nbsp;<span class=\"badge badge-info\">{{task.status | capslock}}</span></div><div style=font-size:1.5em><a href=/task/view/{{task._id}}>{{task.name}}</a></div></div><div class=pull-right></div></div></div><div class=row><div class=col-md-12>{{task.description}}</div></div><div class=row ng-if=\"task.ownerId==_me._id && task.applicants.length==0\"><div class=\"col-md-12 lead\">No one has applied yet. Try sharing your task via the share buttons.</div></div><div class=\"row top-buffer\"><div class=col-md-12><div onrender=rendered() class=fb-comments data-href={{_hostName}}/task/view/{{task._id}}#{{task._id}} data-numposts=3 data-colorscheme=light></div></div></div></div><!--./col-md-7--><div class=\"col-md-5 col-sm-12\"><div class=row><div class=col-md-12><!--share stuff --><!-- Facebook Share Button --><a class=\"btnz share facebook\" ng-click=\"_share('fb','Help '+task.ownerName+' with ' + task.name,task.description)\" href=#><i class=\"fa fa-facebook\"></i> Share</a><!-- Googple Plus Share Button --><!-- <a class=\"btnz share gplus\" href=\"#\"><i class=\"fa fa-google-plus\"></i> Share</a> --> <a href=# ng-click=\"_share('google',task.name,task.description)\"><img src=https://www.gstatic.com/images/icons/gplus-64.png style=height:46px alt=\"Share on Google+\"/></a><!-- Twitter Share Button --> <a class=\"btnz share twitter\" href=\"https://twitter.com/intent/tweet?text={{'Help '+task.ownerName+' with '+task.name+' link: '+currentUrl}}&hashtags=snaptasq\"><i class=\"fa fa-twitter\"></i> Tweet</a><!-- Stumbleupon Share Button --><!-- <a class=\"btnz share stumbleupon\" href=\"#\"><i class=\"fa fa-stumbleupon\"></i> Stumble</a> --><!-- Pinterest Share Button --> <a href=# class=\"btnz share pinterest\" ng-click=\"_share('pintrest','Help '+task.ownerName+' with '+task.name+' link: '+currentUrl,task.description)\"><i class=\"fa fa-pinterest\"></i> Pin it</a><!-- LinkedIn Share Button --><!-- <a class=\"btnz share linkedin\" href=\"#\"><i class=\"fa fa-linkedin\"></i> LinkedIn</a> --><!-- Buffer Share Button --> <a href=https://bufferapp.com/add class=buffer-add-button data-count=horizontal>Buffer</a><script src=https://static.bufferapp.com/js/button.js></script><!-- \n" +
    "        }\n" +
    "        }\n" +
    "        <a class=\"btnz share buffer\" href=\"#\"><i class=\"fa fa-share-square-o\"></i> Buffer</a> --></div></div><div ng-if=task.location class=row id=googleMaps><div ng-if=!_isMobile class=col-md-12><div ui-gmap-google-map fullscreen center=task.locationCopy.geo.center zoom=15><div ui-gmap-marker coords=task.location.geo.center options=marker.options idkey=0></div></div></div><div ng-if=_isMobile class=\"col-sm-6 col-md-6\"><div ui-gmap-google-map center=task.locationCopy.geo.center zoom=15><div ui-gmap-marker coords=task.location.geo.center options=marker.options idkey=0></div></div></div></div><!-- <div class=\"row\">\n" +
    "        <div class=\"col-md-12\">\n" +
  ...
  $templateCache.put('components/modal/modal.html',
    "<div class=modal-header><button ng-if=modal.dismissable type=button ng-click=$dismiss() class=close>&times;</button><h4 ng-if=modal.title ng-bind=modal.title class=modal-title></h4></div><div class=modal-body><p ng-if=modal.text ng-bind=modal.text></p><div ng-if=modal.html ng-bind-html=modal.html></div><div ng-if=modal.htmlInclude ng-include src=modal.htmlInclude></div></div><div class=modal-footer><button ng-repeat=\"button in modal.buttons\" ng-class=button.classes ng-click=button.click($event) ng-bind=button.text class=btn></button></div>"
  );
