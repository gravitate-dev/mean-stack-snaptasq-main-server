'use strict';
angular.module('snaptasqApp').controller('TaskGlobalCtrl', function($scope, $route, localStorageService, Notification, notifications, $location, $window, $routeParams, Auth, Task, $timeout, $interval, User, TaskMock, KeyEventService, TaskMarshaler, Modal, $rootScope) {
    /**
    ...
    /**
     * Notification from the phoneNumber.modal.controller
     * When a user entered a correct verification code
     * Snce I cant force ME to update after, i set a flag here
     * to tell applyToTask that I have already verified my #
     */
    $scope.canStartTask = function(task, me) {
        if (angular.isUndefined(me)) {
            return false;
        }
        if (task.ownerId == me._id) return false;
        // if its started then i cant start it again
        if (!angular.isUndefined(task.startTime)) return false;
    ...
    $scope.canFinishTask = function(task, me) {
        ...
        if (task.ownerId == me._id) return false;
        // if its started then i cant start it again
        if (!angular.isUndefined(task.endTime)) return false;
        if (angular.isUndefined(task.startTime)) return false;
        //check to see if i am in the task.applicants
        if (!angular.isUndefined(task.tasker) && !angular.isUndefined(task.tasker.id)) {
            if (task.tasker.id == me._id) {
    ...
    $scope.canUnapplyToTask = function(task, me) {
         ...
         **/
    $scope.unapplyToTask = function(task, me) {
        Task.unapplyToTask({
            id: task._id
        }, {}, function(data) {
    ...
    $scope.applyToTask = function(task, me) {
        if (!Auth.isLoggedIn()) {
            $scope.connect();
        } else {
            Task.applyToTask({
                ...
                //task.applicants = data.applicants;
                //task.tasker = data.tasker;                
                $scope.showEnterPhoneNumberApplicant(me);
            });
        }
    };
    ...
    $scope.$on('phoneVerified', function() {
        localStorageService.set('hidePhoneNumberModal', true);
    });
    $scope.showEnterPhoneNumberApplicant = function(me) {
        if (localStorageService.get('hidePhoneNumberModal')) {
            return;
        }
        if (me.phone.verified != true && me.phone.ignorePrompt != true) {
            if ($scope.alreadyVerified == false) {
...
}).controller('TaskEditCtrl', function($scope, $window, User, localStorageService, Modal, _me, notifications, $routeParams, Task, Notification, $rootScope, TaskMarshaler, Auth, $location) {
    $scope._noFooter();
    $scope._bgcolorGrey();
    ...
    _me.$promise.then(function(me) {
            $scope._me = me;
            if (me.requiresBeta && window._beta) {
                $location.path('/beta');
            }
        })
        /**
    ...
    $scope.changeStepTo = function(stepName) {
        if (stepName == "finish") {
            if ($scope._me != undefined) {
                $scope.showEnterPhoneNumberOwner($scope._me);
            }
            if ($scope.taskId) {
    ...
    $scope.showEnterPhoneNumberOwner = function(me) {
        if (localStorageService.get("hidePhoneNumberModal")) {
            return;
        }
        if (me.phone.verified != true && me.phone.ignorePrompt != true) {
            Modal.input.phoneNumberOwner(function(data) {
                // called when the user clicks no thank you
                User.set("phone.ignorePrompt", true);
                localStorageService.set("hidePhoneNumberModal", true);
            })();
