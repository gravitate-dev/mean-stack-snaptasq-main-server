...
var UserSchema = new Schema({
  ...
  pic:{type:String,  default:config.host.url+"assets/logos/no_avatar.gif"},
  isConnectedWithFb:{ type: Boolean, default: false},
  hasConnectedWithFbOnce:{ type: Boolean, default: false},
  created: { type: Date, default: Date.now },
  updated: { type: Date, default: Date.now },
  //when this secureThrottle is in the future we block requests
  throttle: {
    forgotPass: {type: Date}, //forgot pass is one time per hour
    verifyEmail: {type: Date} //verify email is throttle one time per hour
  },
  fb: {
    id: String,
    accessToken: String,
  ...
  forgotPassCode:{type:String}, //uuid
  role: {
    type: String,
    default: 'user'
  },
  ...
  provider: String,
  salt: String,
  requiresBeta:{ type: Boolean, default: false}
});
...
var friendSchema = new Schema({
  ...
  name:String, 
  pic:{type: String, default:"assets/logos/no_avatar.gif"},
  fbId:String
});
/**
 * Virtuals
...
UserSchema.static('findByVerificationCode', function (code, callback) {
  return this.model('User').find({ 'verification.code': code }, callback);
});
