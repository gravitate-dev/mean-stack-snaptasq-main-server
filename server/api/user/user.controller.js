...
exports.create = function (req, res, next) {
  ...
  newUser.save(function(err, user) {
    ...
    Notify.put(user._id,"ACCOUNT_CREATE_NEW",{name:user.name},undefined,function(){});
    User.findById(user._id, function (err, user) {
    if (err || user==null) {
      return res.status(500).json({ status: "error", message:"We Couldn't find your account."});
      return;
    }
...
exports.applyBetaCode = function(req, res, next){
  ...
  User.findOne({
      ...
      if (!user) return res.status(500).json({message: "Please login again"});
      if (!user.requiresBeta) return res.status(200).json({message: "Success, you have already entered a beta code!"});
      user.requiresBeta = false;
      user.save(function(err){
        if (err) return validationError(res,err);
        Notify.put(user._id,"BETA_CODE_REGISTER_SUCCESS",{name:user.name},undefined,function(){});
        return next();
      });
    });
}
...
exports.askFriend = function (req, res, next) {
  ...
  if (targetFriendId==undefined)
    return res.status(500).json({message:"Missing friend id to add, failed to add friend."});
...
exports.resetChangePassword = function(req, res, next) {
  ...
  if (code2!=sha1(code1)){
    return res.status(403).json({message:"Incorrect reset code", status:"error"});
  }
  User.findOne({
    "forgotPassCode": code1
  }, function (err, user) {
    if (err || user==null) {
      return res.status(404).json({ status: "error", message:"There is no account with that email address."});
    } else {
      user.password = newPass;
      user.forgotPassCode = uuid.v4();
      user.save(function(err) {
...
exports.sendVerificationEmail = function(req, res, next){
    ...
    if (err || user==null) {
      ...
      return res.status(500).json({ status: "warn", message:"Your email address is already verified"});
    }
  ...
  User.findOne({
    ...
    if (err || user==null) {
      ...
      if (user.forgotPassCode==undefined){
        return res.status(404).json({ status: "error", message:"Test email accounts do not have forgotPasswordCodes"});
      } else {
